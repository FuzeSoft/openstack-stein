...
class RequestHandler(xfr.XFRMixin):
    ...
    def _handle_notify(self, request):
        ...
        * Checks if SOA query response serial != local serial.
        ...
        if not main_addr:
            LOG.warning("NOTIFY for %(name)s from non-main server %(addr)s, "
        ...
        if soa_serial == zone.serial:
            LOG.info("Serial %(serial)s is the same for main and us for "
                     "%(zone_id)s", {"serial": soa_serial, "zone_id": zone.id})
        else:
            LOG.info("Scheduling AXFR for %(zone_id)s from %(main_addr)s",
                     {"zone_id": zone.id, "main_addr": main_addr})
