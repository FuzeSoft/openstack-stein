...
class ImageCache(object):
    ...
    def fetch_image(self, href, dest_path, ctx=None, force_raw=True):
        ...
        if service_utils.is_glance_image(href):
            main_file_name = service_utils.parse_image_id(href)
        else:
        ...
        if force_raw:
            main_file_name = main_file_name + '.converted'
        ...
        with lockutils.lock(img_download_lock_name):
            ...
            LOG.info("Main cache miss for image %(href)s, "
    ...
    def _download_image(self, href, main_path, dest_path, ctx=None,
        ...
        try:
            ...
            msg = (_("Could not link image %(img_href)s from %(src_path)s "
                   ...
                   {'img_href': href, 'src_path': main_path,
...
def _delete_dest_path_if_stale(main_path, dest_path):
    ...
    if (not main_path_exists
            or os.stat(main_path).st_ino != os.stat(dest_path).st_ino):
