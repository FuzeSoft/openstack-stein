...
class BaseMySqlApp(object):
    ...
    def grant_replication_privilege(self, replication_user):
        LOG.info("Granting Replication Subordinate privilege.")
    ...
    def start_subordinate(self):
        LOG.info("Starting subordinate replication.")
        with self.local_sql_client(self.get_engine()) as client:
            client.execute('START SLAVE')
            self._wait_for_subordinate_status("ON", client, 180)

    def stop_subordinate(self, for_failover):
        replication_user = None
        LOG.info("Stopping subordinate replication.")
        with self.local_sql_client(self.get_engine()) as client:
            ...
            self._wait_for_subordinate_status("OFF", client, 180)
            if not for_failover:
    ...
    def stop_main(self):
        LOG.info("Stopping replication main.")
        with self.local_sql_client(self.get_engine()) as client:
